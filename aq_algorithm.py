...
def covers(complex: dict, row: pd.Series) -> bool:
    ...
    for col in complex.keys():
        if not row[col] in complex[col]:
            return False
    return True
...
def delete_covered(rule: dict, df: pd.DataFrame) -> pd.DataFrame:
    ...
    for r in range(df.shape[0]):
        if not covers(complex, df.iloc[r]):
            indexes.append(r)
    return df.iloc[indexes]
